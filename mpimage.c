...
uint8_t getPixelValue(Image* srcImage,int x,int y,int bit,Matrix algorithm){
    ...
    /* use float accumulation to preserve fractional kernel values,
       then clamp to [0,255] before casting to uint8_t */
    float result =
...
void convolute(Image* srcImage,Image* destImage,Matrix algorithm){
    ...
    /* Parallelize the per-pixel work with OpenMP.
       We collapse the row and column loops so the iteration space is evenly
       distributed among threads. Each thread writes distinct destImage pixels
       (no overlapping indices), so there are no race conditions. */
